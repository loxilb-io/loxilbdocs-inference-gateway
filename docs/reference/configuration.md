...
| `kvExactMode`, `kvBlockSize`, `kvHashAlgo`, `kvZmqPort`, `kvWarmupSec`, `kvEngineType`, `kvDpRankCount`, `kvExactApiMode`, `kvModelProfile`, `pdBootstrapPort` | Typed-engine, strict model-profile, and KV-cache routing (`vllm`, `sglang`, `trtllm`, `llamacpp`) | [Configuration Reference §4](../ai-gateway/configuration-reference.md#4-kv-cache-exact-routing) · [Model Profiles and KV-Exact Readiness](../ai-gateway/model-profiles-kv-readiness.md) · [KV-Cache Routing](../ai-gateway/kv-caching.md) |
| `sockMapMode` | Experimental directional sockmap acceleration for eligible plain HTTP/1.1 fullproxy services | [Configuration Reference §1](../ai-gateway/configuration-reference.md#1-core-service-fields) · [Sockmap Acceleration](../operations/sockmap-acceleration.md) |
